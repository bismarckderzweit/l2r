...
def score(X):
    ...
    """
    A = []
    scorefile_path = temp_path + 'scorefile_alpha{}_{}.txt'.format(alpha, tmp_test_y_query[0].split(':')[-1].split()[0])
    restore_path = temp_path + 'restore_alpha{}_{}.txt'.format(alpha, tmp_test_y_query[0].split(':')[-1].split()[0])
...
def loop_query(query_index):
    ...
    restore_path = temp_path + 'restore_alpha{}_{}.txt'.format(alpha, query_id)
    scorefile_path = temp_path + 'scorefile_alpha{}_{}.txt'.format(alpha, query_id)
    ...
    def get_score_matrix1(feature_matrix):
        ...
        with open(temp_path + 'changed_list_sufficiency{}.txt'.format(query_index), 'w') as f:
        ...
        args = ['java', '-jar', 'RankLib-2.12.jar', '-rank',
                temp_path + 'changed_list_sufficiency{}.txt'.format(query_index), '-load', model,
                '-indri', temp_path + 'changed_list_sufficiency_score{}.txt'.format(query_index)]
        subprocess.check_output(args, stderr=subprocess.STDOUT)
        A = ''.join(sorted(open(temp_path + 'changed_list_sufficiency_score{}.txt'.format(query_index)),
                           key=lambda s: int(s.split()[0]), reverse=False))
        with open(temp_path + 'changed_list_sufficiency_score{}.txt'.format(query_index), 'w') as f:
            f.write(A)
        changed_list_score = []
        with open(temp_path + 'changed_list_sufficiency_score{}.txt'.format(query_index), 'r') as f:
            for line in f:
        ...
        os.remove(os.path.join(temp_path, 'changed_list_sufficiency{}.txt'.format(query_index)))
        os.remove(os.path.join(temp_path, 'changed_list_sufficiency_score{}.txt'.format(query_index)))
    ...
    def get_score_matrix2(feature_matrix):
        ...
        changed_list = np.array(changed_list)
        with open(temp_path + 'changed_list_completeness{}.txt'.format(query_index), 'w') as f:
            for i in range(feature_matrix.shape[0] * X_test.shape[1]):
                ...
                f.write(line)
        args = ['java', '-jar', 'RankLib-2.12.jar', '-rank',
                temp_path + 'changed_list_completeness{}.txt'.format(query_index), '-load', model,
                '-indri', temp_path + 'changed_list_completeness_score{}.txt'.format(query_index)]
        subprocess.check_output(args, stderr=subprocess.STDOUT, timeout=2000)
        A = ''.join(sorted(open(temp_path + 'changed_list_completeness_score{}.txt'.format(query_index)),
                           key=lambda s: int(s.split()[0]), reverse=False))
        with open(temp_path + 'changed_list_completeness_score{}.txt'.format(query_index), 'w') as f:
            f.write(A)
        changed_list_score = []
        with open(temp_path + 'changed_list_completeness_score{}.txt'.format(query_index), 'r') as f:
            for line in f:
                changed_list_score.append(float(line.split()[-2]))
        changed_list_score = [changed_list_score[i:i + tmp_test_data.shape[1]] for i in
                              range(0, len(changed_list_score), tmp_test_data.shape[1])]
        os.remove(os.path.join(temp_path, 'changed_list_completeness{}.txt'.format(query_index)))
        os.remove(os.path.join(temp_path, 'changed_list_completeness_score{}.txt'.format(query_index)))
        return changed_list_score
    ...
    def get_matrix(ranked_test_data):
        ...
        for i in range(len(pairsname)):
            ...
            row = [(alpha * round((score_values1[index1 - 1][j] - score_values1[index2 - 1][j]), 10) + (
            1 - alpha) * round((score_values2[index2 - 1][j]
    ...
    def feature_k_loop(feature_number):
        NDCG_file_name = NDCGdata_path + '{}_alpha{}_{}features'.format(dataname, alpha,
                                                                        feature_number) + modelname + '.txt'
        NDCG_file_matrix = NDCGdata_path + '{}_alpha{}_matrix_{}features'.format(dataname, alpha,
                                                                                 feature_number) + modelname + '.txt'
        ranklist_file = NDCGdata_path + '{}_ranklist_alpha{}_{}features'.format(dataname, alpha,
                                                                                feature_number) + modelname + '.txt'
...
if __name__ == '__main__':
    ...
    model_set  =['Ranknet_model.txt','Listnet_model.txt']
    for MODEL in model_set:
        ...
        for alpha in np.arange(0.1,1.0,0.2):
            ...
            # the path of data
                ...
                resultfile_NDCG = 'resultfile/' + '{}_{}_alpha_NDCG.txt'.format(dataname,modelname)
                resultfile_tau = 'resultfile/' + '{}_{}_alpha_tau.txt'.format(dataname,modelname)
                resultfile_ratio =  'resultfile/' + '{}_{}_alpha_ratio.txt'.format(dataname,modelname)
                ...
                for feature_number in (5,10):
                    NDCG_file_name = NDCGdata_path + '{}_alpha{}_{}features'.format(dataname,alpha,feature_number) + modelname + '.txt'
                    NDCG_file_matrix = NDCGdata_path + '{}_alpha{}_matrix_{}features'.format(dataname,alpha,feature_number)  + modelname + '.txt'
                    ranklist_file = NDCGdata_path + '{}_ranklist_alpha{}_{}features'.format(dataname,alpha,feature_number)  + modelname + '.txt'
