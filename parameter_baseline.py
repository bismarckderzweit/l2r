...
def loop_query(query_index):
    ...
    def feature_k_loop(feature_number, threshold_flag):
        top_k_idx = random.sample(range(tmp_test_data.shape[1]), 41)
    ...
    feature_k_loop(41, threshold_flag)


if __name__ == '__main__':
    # parameters to be set
    model_path = 'model/'
    model_set = ['Linearregression_model.txt', 'LambdaMART_model.txt', 'coordinateascent_model.txt',
                 'Randomforest_model.txt', 'Listnet_model.txt', 'Ranknet_model.txt']
    for MODEL in model_set:
        ...
        for f in range(1, 2):
            ...
            NDCG_file_name = NDCGdata_path + '{}_baseline_{}features_threshold{}'.format(dataname, feature_number,
                                                                                         threshold_flag) + modelname + '.txt'
            ranklist_file = NDCGdata_path + '{}_ranklist_baseline_{}features_threshold{}'.format(dataname,
                                                                                                 feature_number,
                                                                                                 threshold_flag) + modelname + '.txt'
