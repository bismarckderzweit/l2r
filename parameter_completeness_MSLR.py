...
def loop_query(query_index):
    ...
    def get_score_matrix(feature_matrix):
        ...
        args = ['java', '-jar', 'RankLib-2.12.jar', '-rank',
                temp_path + 'changed_list_MATRIX{}.txt'.format(query_index), '-load', model,
        ...
        A = ''.join(sorted(open(temp_path + 'changed_list_MATRIX_score{}.txt'.format(query_index)),
    ...
    def get_matrix(ranked_test_data):
        ...
        for i in range(len(pairsname)):
            ...
            row = [round((score_values[index2 - 1][j] - score_values[index1 - 1][j]), 4) for j in
    ...
    def feature_k_loop(feature_number):
        NDCG_file_name = NDCGdata_path + '{}_completeness_{}features'.format(dataname,
                                                                             feature_number) + modelname + '.txt'
        NDCG_file_matrix = NDCGdata_path + '{}_completeness_matrix_{}features'.format(dataname,
                                                                                      feature_number) + modelname + '.txt'
        ranklist_file = NDCGdata_path + '{}_ranklist_completeness_{}features'.format(dataname,
                                                                                     feature_number) + modelname + '.txt'
        top_k_idx = []
        ...
        for i in range(9):
            ...
            temp_matrix = get_matrix(temp_ranked_test_data)
        ...
        restore_path = temp_path + 'restore_completeness_{}.txt'.format(query_id, feature_number)
        scorefile_path = temp_path + 'scorefile_completeness_{}.txt'.format(query_id, feature_number)
...
if __name__ == '__main__':
    ...
    for MODEL in model_set:
        ...
        for f in range(1, 2):
            ...
            temp_path = 'temp_data_completeness/'
            ...
            resultfile_NDCG = 'resultfile/' + '{}_{}_completeness_NDCG.txt'.format(dataname, modelname)
            resultfile_tau = 'resultfile/' + '{}_{}_completeness_tau.txt'.format(dataname, modelname)
            resultfile_ratio = 'resultfile/' + '{}_{}_completeness_ratio.txt'.format(dataname, modelname)
